...
- [SeaSideX ビルドガイド](#seasidex-ビルドガイド)
  - [1. 準備](#1-準備)
    - [1-1. キット同梱品確認](#1-1-キット同梱品確認)
    - [1-2. 組立前にお客様自身で準備いただく部品](#1-2-組立前にお客様自身で準備いただく部品)
      - [1-2-1. バッテリーの選び方と注意点](#1-2-1-バッテリーの選び方と注意点)
    - [1-3. 必要な工具](#1-3-必要な工具)
  - [2. 組み立て](#2-組み立て)
    - [2-1. ポゴピンの取り付け（一部）](#2-1-ポゴピンの取り付け一部)
    - [2-2. スイッチの取り付け](#2-2-スイッチの取り付け)
    - [2-3. マイコンの足をカット](#2-3-マイコンの足をカット)
    - [2-4. バッテリーコネクタの取り付け](#2-4-バッテリーコネクタの取り付け)
    - [2-5. コンスルーの取り付け](#2-5-コンスルーの取り付け)
    - [2-6. ポゴピン残りのはんだ付け（ボール側のみ）](#2-6-ポゴピン残りのはんだ付けボール側のみ)
    - [2-7. マイコンのはんだ付け](#2-7-マイコンのはんだ付け)
    - [2-8. 使用モデルに応じた配線](#2-8-使用モデルに応じた配線)
      - [2-8-1. SeaSide39用配線](#2-8-1-SeaSide39用配線)
      - [2-8-2. SeaSide44用配線](#2-8-2-SeaSide44用配線)
      - [2-8-3. SeaSide39(左手トラボ)用配線](#2-8-3-SeaSide39(左手トラボ)用配線)
    - [2-9. トラックボールセンサーのはんだ付け](#2-9-トラックボールセンサーのはんだ付け)
    - [2-10. LED配線カット](#2-10-led配線カット)
    - [2-11. 組み立て](#2-11-組み立て)
  - [3. ファームウェア](#3-ファームウェア)
    - [3-1. テスト用ファームウェアのダウンロード](#3-1-テスト用ファームウェアのダウンロード)
    - [3-2. テスト用ファームウェアの書き込みとペアリング](#3-2-テスト用ファームウェアの書き込みとペアリング)
    - [3-3. 動作確認](#3-3-動作確認)
    - [3-4. ZMK Studioの活用](#3-4-zmk-studioの活用)
    - [3-5. ZMKファームウェアのカスタマイズとビルド方法](#3-5-zmkファームウェアのカスタマイズとビルド方法)
  - [4. お疲れさまでした](#4-お疲れさまでした)

![DSC02063](https://raw.githubusercontent.com/hama-be/SeaSideX-documentation/main/docs/img/DSC02063.jpg)

> 購入前の注意事項は [`doocs/about.md`](../doocs/about.md) をご確認ください。
...
追記：上記のおすすめバッテリーが2025/07/10現在欠品中のようです。代わりのバッテリーを探す際に重要な観点を記載しましたので参考にしてみてください。[1-2-1. バッテリーの選び方と注意点](#1-2-1-バッテリーの選び方と注意点)

LED配線済のバッテリー持続時間の注意点については「[注意事項: 1-6. LED配線によるバッテリー持続時間の違い](../doocs/about.md#1-6-led配線によるバッテリー持続時間の違い)」セクションをご確認ください。


<a id="1-2-1-バッテリーの選び方と注意点"></a>
#### 1-2-1. バッテリーの選び方と注意点
...
3. 基板の裏面からバッテリー端子用のスルーホールにはんだ付けします。この時、BAT+に赤色、BAT-に黒色を接続します。必ず間違えないように確認してください。（ただし例外的に逆に配線する場合もあります。詳細は[1-2-1. バッテリーの選び方と注意点](#1-2-1-バッテリーの選び方と注意点)をいただき、極性に間違いの無いように配線を行ってください。）\
...
ここで切り落としたコネクタの線は、使用するモデルによって後の[2-8. 使用モデルに応じた配線](#2-8-使用モデルに応じた配線)で使用する場合があります。捨てずに取っておいてください。

<a id="2-5-コンスルーの取り付け"></a>
### 2-5. コンスルーの取り付け
...
<a id="2-6-ポゴピン残りのはんだ付け（ボール側のみ）"></a>
### 2-6. ポゴピン残りのはんだ付け（ボール側のみ）
...
<a id="2-8-使用モデルに応じた配線"></a>
### 2-8. 使用モデルに応じた配線

`SeaSideX`を`SeaSide39`にするか`SeaSide44`にするかを選ぶ配線を行います。対象はボール側の基板のみです。\
左手トラボで使用する場合は少し事情が異なるので[2-8-3. SeaSide39(左手トラボ)用配線](#2-8-3-SeaSide39(左手トラボ)用配線)をよく読み配線を行ってください。
...
[2-4. バッテリーコネクタの取り付け](#2-4-バッテリーコネクタの取り付け)で取っておいたコネクタ線の切れ端を使用し、画像の長さの線を作成します。\
...
<a id="2-8-3-SeaSide39(左手トラボ)用配線"></a>
#### 2-8-3. SeaSide39(左手トラボ)用配線
...
Keyball39左トラボでは、先述の[SeaSide39用配線](#2-8-1-seaside39用配線)に加えて、Keyball39本体のボール側基板背面のLEFTパッドの短絡を外す必要があります。\
以下のようになればOKです。
...
Keyball39ishの左トラボモデルの場合、39ではありますが先述の[SeaSide44用配線](#2-8-2-seaside44用配線)を行ってください。少し気持ち的な違和感があるかもしれませんが、こちらもご了承いただけますと幸いです。


<a id="2-9-トラックボールセンサーのはんだ付け"></a>
### 2-9. トラックボールセンサーのはんだ付け
...
<a id="2-10-LED配線カット"></a>
### 2-10. LED配線カット

[注意事項: 1-6. LED配線によるバッテリー持続時間の違い](../doocs/about.md#1-6-led配線によるバッテリー持続時間の違い)で説明した通り、SeaSideXをLED配線を行っているKeyballで使用する場合、点灯していないLEDの待機電力によってバッテリー持ちが悪くなることが報告されています。\
...
<a id="3-1-テスト用ファームウェアのダウンロード"></a>
### 3-1. テスト用ファームウェアのダウンロード
...
<a id="3-2-テスト用ファームウェアの書き込みとペアリング"></a>
### 3-2. テスト用ファームウェアの書き込みとペアリング
...
<a id="3-4-zmk-studioの活用"></a>
### 3-4. ZMK Studioの活用

[注意事項: 1-3. ZMKファームウェアの使用](../doocs/about.md#1-3-zmkファームウェアの使用)で説明したとおり、ZMK Studioを使用することでファームウェアを書き換えることなくGUIでキーマップを簡単に編集できます。\
...
<a id="3-5-zmkファームウェアのカスタマイズとビルド方法"></a>
### 3-5. ZMKファームウェアのカスタマイズとビルド方法
