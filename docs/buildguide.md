...
ishシリーズに対応するためにはish用の基板（オプション）をご購入いただく必要がありますのでご了承ください。
...
| メイン基板BallSide | 1枚 | |
| メイン基板NoBallSide | 1枚 | |
| メイン基板NoBallSide ishシリーズ用 | 1枚 | ishシリーズで使用する場合は片側のみ専用基板（オプション）を使用 |
...
また、Keyballish版を組み立てる場合、ボール無し側基板はSeaSideXishと書かれているものを使用してください。組み立て工程は通常版と同じです。\
